...
void send_json_frame(int fd, std::string_view kind, std::string_view payload) {
...
int main() {
    ...
    const auto temp_root = icss::testsupport::make_temp_configured_repo("icss_server_process_live_json_");
    ...
    send_json_frame(tcp_client.fd, "session_join", serialize(SessionJoinPayload{{1001U, 101U, 1U}, "command_console"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);

    send_json_frame(tcp_client.fd, "scenario_start", serialize(ScenarioStartPayload{{1001U, 101U, 2U}, "basic_intercept_training"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);

    send_json_frame(tcp_client.fd, "track_request", serialize(TrackRequestPayload{{1001U, 101U, 3U}, "target-alpha"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);

    send_json_frame(tcp_client.fd, "asset_activate", serialize(AssetActivatePayload{{1001U, 101U, 4U}, "asset-interceptor"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);
    ...
    send_json_frame(tcp_client.fd, "scenario_stop", serialize(ScenarioStopPayload{{1001U, 101U, 7U}, "process smoke stop"}));
    const auto stop_frame = wait_for_json_frame(tcp_client.fd);
