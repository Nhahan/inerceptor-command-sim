...
namespace icss::net {
namespace {
using namespace icss::net::detail;
#if !defined(_WIN32)
class SocketHandle {
public:
...
private:
    ...
    void process_tcp_frame(const icss::protocol::FramedMessage& frame) {
        ...
        if (kind == "scenario_start") {
            ...
            scenario.launch_angle_deg = payload.launch_angle_deg;
        ...
        if (kind == "track_drop") {
            ...
            if (payload.target_id != kTargetId) {
                send_ack(payload.envelope,
                         reject_payload("Track release rejected",
                                        "track release target does not match the active target",
                                        {std::string{kTargetId}}));
                return;
            }
            const auto result = session_.release_track();
            send_ack(payload.envelope, result);
            send_pending_snapshots();
            return;
        }
